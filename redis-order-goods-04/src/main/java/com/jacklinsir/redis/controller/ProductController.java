...
public class ProductController {
    ...
    public CommonResults getByRedis(@RequestParam("userId") final Integer userId) {
        try {
    ...
    @ResponseBody
    @GetMapping("/getByMysql")
    public CommonResults getByMysql(@RequestParam("userId") final Long userId) {
        try {
            List<Product> historyProducts = productService.getProducts(userId);
            return historyProducts.size() > 0 ? CommonResults.success(historyProducts) : CommonResults.failure();
        } catch (Exception e) {
            e.printStackTrace();
            log.info("查询商品列表发生异常: {}", e.fillInStackTrace());
